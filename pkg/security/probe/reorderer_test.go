...
func TestOrderRetention(t *testing.T) {
	...
	var count int
	heap.dequeue(func(record *perf.Record) { count++ }, 1, &metric)
	assert.Equal(t, 30, count)
	heap.dequeue(func(record *perf.Record) { count++ }, 2, &metric)
	assert.Equal(t, 60, count)
	heap.dequeue(func(record *perf.Record) { count++ }, 3, &metric)
...
func TestOrderGeneration(t *testing.T) {
	...
	heap.enqueue(&record, uint64(10), uint64(1), &metric)

	record = perf.Record{
		RawSample: []byte{byte(1)},
	}
	heap.enqueue(&record, uint64(1), uint64(2), &metric)

	var data []byte
	heap.dequeue(func(record *perf.Record) { data = record.RawSample }, 1, &metric)
	assert.Equal(t, 1, int(data[0]))

	heap.dequeue(func(record *perf.Record) { data = record.RawSample }, 2, &metric)
